...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('argentdefinir')
    .setDescription('Definir le nombre de diamants à un membre')
  ...
  async execute(interaction) {
    ...
    const fields = [
      ...
      { name: 'Montant du portefeuille', value: `${amount} 💎`, inline: true },
    ];
    if (reason) {
      fields.push({ name: 'Raison du don', value: reason, inline: true });
    }

    await interaction.reply({ embeds: [embed(interaction, fields, 'Vous avez bien défini des diamants à un membre')] });
    logger.info(
      `${interaction.member.user.globalName} a défini ${amount} diamants à ${user.username} | Raison : ${reason} `
    );
  },
};
